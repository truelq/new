...
void splay(int x, int goal) //将节点x旋成goal的子节点
{
  ...
  if (goal ==
    ...
    root =
        x; // root才是整颗树的根
           // ，因为后面要用到root这个变量，所以，x到root的位置后，要将root改为x
}
...
void Init()//左哨兵和右哨兵分别位于整个序列两边
{
  tot = 0;
  root = newnode(0, -1, 0);     //在0下建一个值为-1的节点，暂时叫root ///////////0/////////////
  int x = newnode(root, -1, 1); //在root下建一个值为-1的右儿子       ///////////|/////////////
  for (int i = 0; i < n; i++)   //原始数组                         ///////////root(-1)//左哨兵
    a[i] = i + 1;                                                     ///////////////\//////////
  build(x, 0, n - 1, 0);        //在x的左儿子建整个序列             /////////////////son[1](-1)x//右哨兵
  update(T[root].son[1]);                                          //\\\\\\\\\\\\\\\\/\\\\\\\\\\\\\\//
  update(root);                                                    /////////Key_value(序列)/////////
}
void addrevtag(int l, int r) //反转序列区间[l,r] ，编号从1开始
{
  int gl = getkth(root,l); //树中的第l个元素，实际上是序列中的第l-1个元素（因为多了一个左哨兵）
  // cout<<gl<<endl;
  ...
  // cout<<"ok"<<endl;
  updrev(keyvalue);          //那么Key_value的位置就是区间【l,r】,将翻转懒人标记作用在Key_value上
